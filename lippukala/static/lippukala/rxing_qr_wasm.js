 ...
 * @returns {string}
 */
export function decode_qrcode_text(luma, width, height, try_harder) {
  let deferred3_0;
  let deferred3_1;
  try {
    const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
    const ptr0 = passArray8ToWasm0(luma, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    wasm.decode_qrcode_text(retptr, ptr0, len0, width, height, isLikeNone(try_harder) ? 0xffffff : try_harder ? 1 : 0);
...
async function __wbg_init(input) {
  ...
  if (typeof input === "undefined") {
    input = new URL("rxing_qr_wasm_bg.wasm", import.meta.url);
  }
  const imports = __wbg_get_imports();
